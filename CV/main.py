...
Modified the work of Alireza Bagheri (https://github.com/alireza787b/Python-Gaze-Face-Tracker)
who was in turn inspired by Asadullah Dal's iris segmentation project (https://github.com/Asadullah-Dal17/iris-Segmentation-mediapipe-python). 
...
import os
from CV.AngleBuffer import AngleBuffer
import threading
...
def record():
    global EYES_BLINK_FRAME_COUNTER, TOTAL_BLINKS
        ...
        while IS_RECORDING.is_set():
            ...
            if results.multi_face_landmarks:
                ...
                if LOG_DATA:
                    ...
                    # log_entry = [timestamp, l_cx, l_cy, r_cx, r_cy, l_dx, l_dy, r_dx, r_dy, TOTAL_BLINKS, forward] 
                    log_entry = [timestamp, TOTAL_BLINKS, forward] 
...
def start_record():
    ...
    if not IS_RECORDING.is_set():  # Start only if not already recording
        IS_RECORDING.set()  # Set the event to True
        recording_thread = threading.Thread(target=record)
        recording_thread.start()

def stop_record():
    global IS_RECORDING
    if IS_RECORDING.is_set():  # Stop only if currently recording
        IS_RECORDING.clear()  # Set the event to False
